...
class UartState:
    ...
    def handle_new_byte(self, byte_val):
        ...
        if self.last_byte_val not in [BLOCK_ERASE_FAIL, BLOCK_TEST_START, PROG_INCREMENT, READ_INCREMENT, BLOCK_TEST_COMPLETE, TEST_BYTE_CHANGED_NOTIFICATION] and self.is_line_busy:
...
def log_terminal(message="\n", file=sys.stdout, omit_timestamp=False):
    """
    ...
    """
    timestamp = f"[{get_timestamp()}]  " if message != "\n" and not omit_timestamp else ""
...
def update_block_test_display(state):
    ...
    padding = " " * 10
    message = (
        f"\rBlock {state.current_block:<5} Checking pages... "
        f"BYTE 0x{state.current_test_byte_key} | PROG: {prog_count:>3} | READ: {read_count:>3} | {total_time:.3f} s{padding}"
    )
        ...
        while True:
            ...
            if state.capture_mode:
                ...
                if state.bytes_to_capture == 0:
                    ...
                    # Output message
                    message = (
                        f"Block {state.current_block:<5} BAD BYTE at 0x{address:04X} ({address:>4}) "
                        f"expected 0x{state.current_test_byte_key} actual 0x{data:02X}\n"
                    )
                    log_terminal(message)
                    state.bad_bytes_in_block += 1
            ...
            if byte_val == INIT_COMPLETE:
            ...
            elif byte_val == BLOCK_TEST_START:
                ...
                if state.erase_failed == False:
                    ...
                    if (state.bad_bytes_in_block == 0
                        ...
                        msg = f"\rBlock {state.current_block:<5} OK   {(time_AA + time_55):.3f}s   0xAA {time_AA:.3f}s   0x55 {time_55:.3f}s" + " " * 100 + "\n"
                        log_terminal(msg)
                        state.is_line_busy = True
                    else:
                        log_terminal(f"\rBlock {state.current_block:<5} FAIL   {(time_AA + time_55):.3f}s   Bad bytes: {state.bad_bytes_in_block}   0xAA {time_AA:.3f}s PROG: {stats['AA']['prog']} READ: {stats['AA']['read']}   0x55 {time_55:.3f}s PROG: {stats['55']['prog']} READ: {stats['55']['read']}\n")
